...
function Beg2() {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const response = await fetch('/topicsA1-2.json'); // تعديل المسار إذا لزم الأمر
          ...
          <CSSTransition key={index} classNames="fade" timeout={500}>
            <Col xs={12} md={6} lg={4} key={index} className='mb-5'>
              <Card className="h-100 topic-card">
                <Card.Header>
                  <Link to={`/levels/beg-2/${topic.title.toLowerCase().replace(/\s+/g, '-')}`} className="text-decoration-none stretched-link">
                    {topic.title}
                  </Link>
                </Card.Header>
              </Card>
