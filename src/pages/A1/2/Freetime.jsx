...
function FreeTimePage() {
  ...
  useEffect(() => {
    const fetchFreeTimeData = async () => {
      try {
        const response = await fetch('/freetime.json'); // Adjust the path if necessary
        const data = await response.json();
        setFreeTimeData(data);
      } catch (error) {
        console.error('Error fetching FreeTime data:', error);
      }
    };
